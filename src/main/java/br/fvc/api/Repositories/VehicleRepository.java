...
public interface VehicleRepository extends JpaRepository<Vehicle, Long> {
    ...
    @Query("SELECT NEW br.fvc.api.Domain.Vehicle.HomeResponseDTO(v, COUNT(v.placa) AS quantidade_total) FROM veiculo v INNER JOIN v.modelo AS m INNER JOIN v.agencia AS a LEFT JOIN reserva AS r ON  r.veiculo = v AND r.data_inicio_aluguel <= :start AND r.data_fim_aluguel >= :end WHERE r.id IS NULL AND a.id = :id_agency AND v.disponivel > FALSE GROUP BY m.nome")
